...
// To test color and set IsCaptured
void Test(Square squaresArray[], int index, sf::Color colorOfCaptured){
    if (squaresArray[index].getColor() == colorOfCaptured){
        squaresArray[index].setIsCaptured(true);
    }
}

// 
// *** Arvan: Use camelCase for variable and function names.
void CaptureAdjacent(Square squaresArray[], int arraySize, sf::Color colorOfCaptured, int level){
    for( int i=0; i<arraySize; i++){
        if ( squaresArray[i].getIsCaptured() == true ){            
            ...
            if(i==0){
                Test(squaresArray, right, colorOfCaptured);
                Test(squaresArray, bottom, colorOfCaptured);
                continue;
            }
            if((i%level==level-1)&&(i<level)){
                Test(squaresArray, left, colorOfCaptured);
                Test(squaresArray, bottom, colorOfCaptured);
                continue;
            }
            if(i==arraySize-1){
                Test(squaresArray, top, colorOfCaptured);
                Test(squaresArray, left, colorOfCaptured);
                continue;
            }
            if((i%level==0)&&(i>=arraySize-level)){
                Test(squaresArray, top, colorOfCaptured);
                Test(squaresArray, right, colorOfCaptured);
                continue;
            }
...
{	
		...
		int level = 3;     // level is number of squares per side.  There are level * level number of squares.
    	int side = level+1; // side is just level + 1. It is used only to initialize arraysize with the correct value.
		int arraySize = side*side;
        ...
        for(int i = 0; i<level*level; i++){
            if(squaresArray[i].getColor() != squaresArray[0].getColor()){
                isSame = false;
                break;
            }
        }
		...
		if( settingUpBoard == true) {
            ...
            for( int i=0; i<arraySize; i++) {              
                char name[ 81];
                sprintf(aString, "par: %d, moves: %d", par, moves); // put par and moves in aString
                squaresArray[i] = Square(WindowXSize/level, (i%level)*(WindowXSize/level) , (i/level)*(WindowXSize/level), getRandomColor(), true, name);           
            }
        ...
        // Setting the 0th square as captured.
        sf::Color colorOfCaptured = squaresArray[0].getColor();
        squaresArray[0].setIsCaptured(true);
        CaptureAdjacent(squaresArray, arraySize, colorOfCaptured, level);
            ...
            // sprintf( aString, "Clicking at %d,%d is square ", localPosition.x, localPosition.y);  // Make a string with mouse x,y values		
						// Using the mouse x,y position, find which square was clicked upon.
						// ...
						int indexOfClicked = (localPosition.x/(WindowXSize/level)) + (level*(localPosition.y/(WindowXSize/level)));
            
            // char colorstr[81];
            // getColorAsString(squaresArray[indexOfClicked].getColor(), colorstr);
            // sprintf( aString, "Clicking at %d,%d is %s square %d", localPosition.x, localPosition.y, colorstr, indexOfClicked);  
            sprintf(aString, "par: %d, moves: %d", par, moves);
            
            // Retrieve the color of the selected square
						// ... // DONE
						sf::Color colorOfClicked = squaresArray[indexOfClicked].getColor();
            
						// Go through the board and capture all adjacent squares of the selected color.
						// ... 						
            for(int i = 0; i < arraySize; i++){
                if( squaresArray[i].getIsCaptured() == true){
                    squaresArray[i].setColor(colorOfClicked);
                }
            }
